...
fig_ridges <- zzz %>% 
  ...
  geom_density_ridges(aes(color = time, fill = time), alpha = .6, quantile_lines = TRUE, quantiles = 2) + 
  labs(x = "Average temperature", y = "Species number", color = "Time", fill = "Time") + 
  scale_fill_manual(values = c("firebrick2","navy")) +
  scale_color_manual(values = c("firebrick2",'navy')) +
  #geom_vline(xintercept= median(zabovef$bio_1), size = 0.75, color = "firebrick2", lty=2) +
  #geom_vline(xintercept= median(zabovep$bio_1), size = 0.75, color = "navy", lty=2) +
  theme_bw(base_size = 15)
...
figreold <- zzz %>% 
  ...
  geom_density_ridges(aes(point_color = time, point_fill = time, point_shape = time),
                      alpha = .4, point_alpha = 0.7, jittered_points = TRUE) +
  scale_point_color_hue(l = 40) +
  scale_discrete_manual(aesthetics = "point_shape", values = c(21, 22))+
  scale_fill_manual(values = c("firebrick2","cyan2")) +
  scale_color_manual(values = c("firebrick2",'cyan2')) +
...
figreold
  ...
  setwd(here())
setwd('hotspots')
fig_ridges <- paste0("Fig_ridges_SSP585only_", version_suffix, ".png")

ggsave(filename = fig_ridges, width = 30, height = 32, units = "cm", dpi = 600)
#gridExtra::grid.arrange(mapdifw, mapdifa, ncol = 2)
...
figre245 <- zall %>% 
  ...
  mutate(time = str_to_title(time)) %>% 
  ggplot(aes(x = bio_1, y = factor(layer), fill = time)) + 
  geom_density_ridges(aes(color = time, fill = time), alpha = .6, quantile_lines = TRUE, quantiles = 2) + #, quantile_lines = TRUE, quantiles = 5
  scale_fill_manual(values = c("firebrick2","gold3", "navy"), labels = c("Future SSP585", "Future SSP245", "Present")) +
  scale_color_manual(values = c("firebrick2","gold3", 'navy'), labels = c("Future SSP585", "Future SSP245", "Present")) +
  labs(x = expression(paste('Average temperature (',~degree,'C)',sep='')), y = "Potential richness", color = "", fill = "" ) + 
  theme_bw(base_size = 15)


figre245

ggsave(filename = 'Fig4_ridges_SSP245_SSP585.png', width = 32, height = 36, units = "cm", dpi = 600, 
       figre245)
