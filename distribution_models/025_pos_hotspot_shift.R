...
setwd('CanESM5_ssp585_2081-2100_27kms/')

setwd('Ensemble/W_MEAN/MAX_TSS/')

fut <- stack(list.files(pattern='.tif'))

futm <- mask(fut, all_aa)
...
setwd(projdir)
setwd('hotspots')
figcentroids <- paste0('Figure_4_inset_', version_suffix, '.jpg')
ggsave(filename = figcentroids, width =25, height = 22, units = 'cm', dpi=600,
       mapdifa)
...
figcomplarge <- paste0('Figure_4_world_', version_suffix, '.jpg')
gridExtra::grid.arrange(maprdf, mapfuture, mapdifa, ncol =3)
ggsave(filename = figcomp, width =35, height = 29, units = 'cm', dpi=600,
       gridExtra::grid.arrange(maprdf, mapfuture, ncol =2))


figcompz <- paste0('Figure_4_zoom_', version_suffix, '.jpg')
ggsave(filename = figcompz, width =38, height = 29, units = 'cm', dpi=600,
gridExtra::grid.arrange(zpresent,zfuture,mapdifa, ncol =3))

figcomp2 <- paste0('Fig_4_zoom_PRESENT_FUTURE_', version_suffix, '.jpg')
...
#writeRaster(richnessm, filename= 'richnessm.tif', format= 'GTiff', overwrite=TRUE)
#writeRaster(richnessfm, filename= 'richnessfm.tif', format= 'GTiff', overwrite=TRUE)
...
mapeurope <- ggplot() + 
  ...
  ylab('Latitude') + xlab('Longitude')+
  theme(plot.background = element_rect(fill=NA, color=NA),
        legend.title=element_blank(), plot.title=element_text(size=14, 
                                                              face = "italic"))+
  theme_bw()
...
figcomposquare <- paste0('Fig_4_europe_SEasia_', version_suffix, '.jpg')
...
mapdifw <- ggplot() + 
  ...
  scale_fill_gradientn(colours = paldif, breaks=c(-8,-5,-2, 0 ,2, 5)) + #
  geom_rect(data = data.frame(), aes(xmin = 95,  
                                     xmax = 106,
                                     ymin =16, 
                                     ymax = 27),
...
figworldbrown <- paste0('Fig_world_spshift_BROWN_', version_suffix, '.jpg')

ggsave(filename = figworldbrown, width =30, height = 22, units = 'cm', dpi=600,
      mapdifw)

gridExtra::grid.arrange(mapdifw, mapdifa, ncol =2)

figshift <- paste0('Fig_world_inset_shift_poster_BROWN_', version_suffix, '.jpg')
...
ggplot(aes(x = bio_1, y =factor(layer)  , fill=time)) +
  geom_density_ridges(aes(point_color = time, point_fill = time, point_shape = time),
                      alpha = .2, point_alpha = 0.9, jittered_points = TRUE) +
  #scale_fill_viridis_d(name = "Hosts")+
  scale_point_color_hue(l = 40) +
  scale_discrete_manual(aesthetics = "point_shape", values = c(21, 22))+
  scale_alpha(guide = 'none')+
  labs(x='Average temperature', y= 'Species number')+
  theme_bw()
